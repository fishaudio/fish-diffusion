...
class BasePitchExtractor:
    ...
    def post_process(self, x, sampling_rate, f0, pad_to):
        ...
        f0 = f0.index_select(dim=0, index=nzindex)
        time_org = self.hop_length / sampling_rate * nzindex
        time_frame = torch.arange(pad_to) * self.hop_length / sampling_rate
        ...
        f0 = torch.interp(time_frame, time_org, f0, left=f0[0], right=f0[-1])

        return f0.to(x.device)
