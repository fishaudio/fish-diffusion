# This file is mostly copied from https://github.com/RVC-Project/Retrieval-based-Voice-Conversion-WebUI/blob/main/lib/rmvpe.py
# License: MIT
...
class RMVPitchExtractor(BasePitchExtractor):
    def __init__(
        self,
        hop_length: int = 512,
        f0_min: float = 50.0,
    ...
    def __call__(self, x, sampling_rate=44100, pad_to=None):
        ...
        if sampling_rate != 16000:
            x = AF.resample(x, sampling_rate, 16000)

        mel = self.mel_extractor(x, center=True)
        hidden = self.mel2hidden(mel)
        hidden = hidden.squeeze(0)
        f0 = self.decode(hidden.cpu().numpy(), threshold=self.threshold)
