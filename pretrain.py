...
def pretrain_ckpt():
    ...
    checkpoint['loops'] = {
        'fit_loop': {
            ...
            'epoch_loop.scheduler_progress': {'total': {'ready': 0,
                    'completed': 0}, 'current': {'ready': 0,
                    'completed': 0}},
            ...
            'epoch_loop.batch_loop.optimizer_loop.optim_progress': {'optimizer': {'step': {'total': {'ready': 0,
                    'completed': 0}, 'current': {'ready': 0,
                    'completed': 0}},
                    'zero_grad': {'total': {'ready': 0, 'completed': 0,
                    'started': 0}, 'current': {'ready': 0,
        ...
        'validate_loop': {
            'state_dict': {},
            'dataloader_progress': {'total': {'ready': 0,
                                    'completed': 0},
                                    'current': {'ready': 0,
                                    'completed': 0}},
...
parser = \
    argparse.ArgumentParser(description='Prepare data to use alongside a pretrain'
                            )
...
if answer.lower() == 'y':
    ...
    print ('Quiting without training...')
