...
class SvcFish:
    ...
    def infer(self, input_path, pitch_adjust, speaker_id):
        return inference(config=self.config, checkpoint=self.checkpoint_path, input_path=input_path, output_path=None,
            speaker_id=speaker_id, pitch_adjust=pitch_adjust, silence_threshold=self.silence_threshold,
            max_slice_duration=self.max_slice_duration, extract_vocals=self.extract_vocals,
            merge_non_vocals=self.merge_non_vocals, vocals_loudness_gain=self.vocals_loudness_gain,
            sampler_interval=self.sampler_interval, sampler_progress=True, device="cuda", gradio_progress=None)
