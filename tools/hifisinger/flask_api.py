...
class HiFiSingerSVC:
    def __init__(self, config_flask):
        ...
        self.hifisinger_model = HiFiSingerSVCInference(self.config, self.checkpoint_path)
    ...
    def _parse_speaker(self, speaker):
        ...
        if hasattr(self.config, "speaker_mapping") and speaker in self.config.speaker_mapping:
        ...
        logger.info(f"Speaker mix: {speaker} -> {[f'{s[0].item()}:{s[1]}' for s in speaker_mix]}")
    ...
    def infer(self, wav_path, pitch_change, speak_id, safe_prefix_pad):
        if safe_prefix_pad > 0.03:
            safe_prefix_pad = safe_prefix_pad - 0.03
        else:
            safe_prefix_pad = 0
        ...
        if safe_prefix_pad != 0:
            ...
            pitches = self.hifisinger_model.pitch_extractor(unpadded, sr, pad_to=unpadded_mel_len).float().cpu()
            if self.pad_f0_model == 'first':
                logger.info("f0_pad=first")
                self.pad_f0 = pitches[0]
            _pitches_pad = torch.full((mel_len - unpadded_mel_len,), self.pad_f0)  # self.last_f0)
            ...
            pitches = self.hifisinger_model.pitch_extractor(audio, sr, pad_to=mel_len).float()
        if (pitches == 0).all():
        ...
        out_wav = self.hifisinger_model.model.generator(
