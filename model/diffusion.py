...
class GaussianDiffusion(nn.Module):
    ...
    def sampling(self):
        ...
        for i in tqdm(reversed(range(0, t)), desc="sample time step", total=t):
            x = self.p_sample(
                x, torch.full((b,), i, device=device, dtype=torch.long), self.cond
            )
        x = x[:, 0].transpose(1, 2)
    ...
    def forward(self, mel, cond, mel_mask):
        ...
        output = epsilon = None
        loss = t = torch.tensor([0.0], device=device, requires_grad=False)
...
class GaussianDiffusionShallow(nn.Module):
    ...
    def expected_kld_t(self, x_pred, x_gt, t, mask):
        ...
        coef = extract(
            self.alphas_cumprod / (2 * self.log_one_minus_alphas_cumprod.exp()),
            t,
            x_pred.shape,
        )
        kld = F.mse_loss(
            self.noised_mel(x_pred, t), self.noised_mel(x_gt, t), reduction="none"
        )
